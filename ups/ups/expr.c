...
char ups_expr_c_sccsid[] = "@(#)expr.c	1.39 04 Jun 1995 (UKC)";
...
int
show_var(sw, fil, lnum, wholename, out_f)
Srcwin *sw;
fil_t *fil;
int lnum;
	    ...
	    strstr(wholename, "::")))
	{
	  for (nptr = names + 1; *nptr != NULL; ++nptr)
	  {
		...
		oldf = errf_set_ofunc(out_f);
	      if (!find_func_by_name(class_name, &f0, &f1))
	      {
...
/*	      have_var = search_stack_files(sw, names[0]);*/
	    ...
	    !(strchr(wholename, '.') || strstr(wholename, "->")))
	{
	  ...
	  if (out_f)
	    oldf = errf_set_ofunc(out_f);
	  if (method && !find_func_by_name(class_name, &f0, &f1))
	  {
	    show_func_source(sw, f0);
	    have_var = 1;
	  }
...
const char *name;
{
	...
	if (*name != sepc && *name != '@' && (*name != '-' || name[1] != '>')) {
		if (!show_local(sw, (func_t *)NULL, 0, name)) {
...
bool
display_local(par, f, lnum, name, restoring, p_var_obj)
objid_t par;
func_t *f;
int lnum;
const char *name;
...
show_global(sw, srcfil, srcfunc, srccblock, name,
	    want_errmesg, from_typing_line, restoring, p_var_obj, undef_check)
Srcwin *sw;
fil_t *srcfil;
func_t *srcfunc;
common_block_t *srccblock;
const char *name;
bool want_errmesg, from_typing_line, restoring, undef_check;
objid_t *p_var_obj;
{
	fil_t *fil, *fil1;
	func_t *f, *f1;
	...
	if (find_global_by_name
	    (name, srcfil, srcfunc, srccblock,
	     !from_typing_line, &f, &f1, &v, &v1, &cblock, &fil, &fil1) != 0)
		return FALSE;
	...
	if (cblock != NULL) {
		...
		add_source_file_object_if_required(fil, FALSE, FALSE);
		obj = (objid_t)fil;
	}
	else
		obj = GLOBALS_OBJCODE;
       ...
       if (fil1 != NULL) {
		add_source_file_object_if_required(fil1, FALSE, FALSE);
		obj1 = (objid_t)fil1;
	}
