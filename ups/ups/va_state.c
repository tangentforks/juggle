...
char ups_va_state_c_sccsid[] = "@(#)va_state.c	1.8 04 Jun 1995 (UKC)";
...
static void update_vf PROTO((Vfile *vf));
...
/* RGA	if (vf != NULL && (force || vf->been_displayed)) {*/
...
int block_start, block_end;
{
	...
	case OT_SFILE:
		show_global((Srcwin *)NULL, (fil_t *)par, (func_t *)NULL,
			    (common_block_t *)NULL,
			    vv->name, FALSE, FALSE, TRUE, &obj, FALSE);
		break;
...
FILE *fp;
{
	...
	for (vs = Vslist; vs != NULL; vs = vs->next) {
		...
		/* C++ names */
		fprintf(fp, "\nfunction \"%s\"", f->fu_demangled_name);
...
int *p_lnum;
{
	...
	if (nargs < 1 || nargs > 3) {
		errf("Usage: function funcname [filename [lnum]] {");
		return -1;
	}
...
int *p_lnum;
{
	...
	if (strcmp(cmd, "globals") == 0) {
		...
		if (nargs == 2 || find_func_by_name(args[1], &f, &f1) != 0)
			f = NULL;
