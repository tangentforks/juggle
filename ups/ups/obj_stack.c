...
char ups_obj_stack_c_sccsid[] = "@(#)obj_stack.c	1.21 20 Jun 1995 (UKC)";
...
*/
const char Func_format[] = "%[-]:4c:50cp%25cp%[-]10cs  \n";
...
fval_t *fields;
{
	...
	if ((f->fu_flags & FU_NOSYM) != 0) {
		...
		fields[FN_FUNC_FILE] = (fval_t)"(no line numbers)";
		fields[FN_FUNC_LNUM] = (fval_t)strsave("");
	}
	else {
...
objid_t code;
{
	...
	return strf("%-20s%25s%-10s", funcname, filename, lnumstr);
}
  ...
  while (stk != NULL)
  {
	...
	stk1 = stk1->stk_inner;
	  ...
	  ((Srcwin *)sw_id, stk->stk_func->fu_fil, (func_t*)NULL,
	   (common_block_t *)NULL,
	   name, TRUE, FALSE, FALSE, (objid_t *)NULL, TRUE))
	return TRUE;
    }
    if (stk == lim)
      break;
