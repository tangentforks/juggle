...
char ups_trun_j_c_sccsid[] = "@(#)trun_j.c	1.32 24 May 1995 (UKC)";
...
stopres_t *p_stopres;
{
	...
	if (target_process_exists(xp) && install_all_breakpoints(xp) != 0)
		return;
		...
		if (l->ln_addr == pc)
		{
		  for (k = 1; l->ln_num - k >= last_l; k++)
		  {
			...
			if (get_varname(f->fu_language, c, d, &start, &lim))
			{
			  ...
			  find_func = find_func_by_name(buff, &f_step, &f_step1);
			  errf_set_ofunc(oldf);
			  if (!find_func &&
