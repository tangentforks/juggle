...
char ups_ui_layout_c_sccsid[] = "@(#)ui_layout.c	1.32 20 Jun 1995 (UKC)";
...
int split;
{
	...
	re_set_mhcaps(regs[TEXT], "select", "edit", "menu");

	re_set_callbacks(regs[NAMEWIN], (re_draw_proc_t)NULL,
...
redo_displayed_source_files()
{
  ...
  obj = code_to_obj(path_to_obj("srchead"));
  for (ob = obj->ob_child; ob; ob = ob->ob_next)
    if (!ob->ob_child)
      break;
  if (!(!obj->ob_child || !ob))
  {				/* source files displayed */
    do_srchead((objid_t)obj->ob_code, MR_HIDE_SOURCE_FILES, (char *)NULL);
    do_srchead((objid_t)obj->ob_code, MR_SHOW_SOURCE_FILES, (char *)NULL);
  }
}
