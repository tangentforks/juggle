...
char ups_va_format_c_sccsid[] = "@(#)va_format.c	1.3 04 Jun 1995 (UKC)";
...
bool is_hint;
{
	...
	if (funcstr == NULL) {
		...
		if (fil != NULL) {
			if ((f = name_and_fil_to_func(funcstr, fil, TRUE)) == NULL) {
				if (find_func_by_name(funcstr, &f, &f1) != 0)
					return -1;;
			}
		}
		else {
...
Format *ft;
{
	...
	if (ft->fil != NULL) {
		...
		if (ft->func != NULL)
			fprintf(fp, ":%s", ft->func->fu_demangled_name);
	}
	else {
		fputs((ft->func != NULL) ? ft->func->fu_demangled_name : "*", fp);
	}
	fputc(' ', fp);
