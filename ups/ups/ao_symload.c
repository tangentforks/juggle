...
char ups_ao_symload_c_sccsid[] = "@(#)ao_symload.c	1.9 05 Jun 1995 (UKC)";
...
static aggr_or_enum_def_t *fix_undef_aggr PROTO((aggr_or_enum_def_t *uae, 
                                                 const char *arg));
static void fix_register_params PROTO((func_t *f));
...
#if !defined(COFF_SUN386) && !defined(AO_ELF)
     ...
     func_t *f;
{
   ...
   */
#ifdef COFF_SUN386
  f->fu__lnos = read_func_coff_lnos(symio, f, &last);
#else
  last = &dummy_lno;
  max_lnum = 0;
