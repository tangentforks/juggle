...
char ups_ci_lex_c_sccsid[] = "@(#)ci_lex.c    1.25 21 Jun 1995 (UKC)";
...
static bool convert_number PROTO((const char *str, int radix,
		  char **p_endstr, unsigned long *p_val));
...
yylex()
{
    ...
    case '5': case '6': case '7': case '8': case '9':
	{
		...
		constant_t *co;
		int i;
		bool is_unsigned, is_long;

		line = end;
		/* IDE/Dibyendu (19/12/98) recognise 'lLuU' */
		is_unsigned = FALSE; is_long = FALSE;
		for (i = 0; i < 2; i++) {
		    if (*line == 'l' || *line == 'L') {
			line++;
			is_long = TRUE;
