 ...
 * Dibyendu 26/12/98
 */
char ups_ci_compile_expr_c_sccsid[] = "@(#)ci_compile_expr.c	1.27 04 Jun 1995 (UKC)";
...
typecode_t typecode;
{
	...
	case TY_LONGLONG:
	case TY_ULONGLONG:
		return DOUBLE_FORM_OFFSET;	/* FIX: unlikely to be right */
#endif
	case TY_LONG:
	case TY_ULONG:
	case TY_INT:
...
expr_context_t context;
{
	...
	if (context == EC_VALUE) {
		switch (typecode) {
		case TY_LONGDOUBLE:
			ci_code_opcode(tx, OC_PUSH_DOUBLE_RETVAL); /* FIX: */
			break;
		case TY_DOUBLE:
...
expr_context_t context;
{
	...
	case OP_UNARY_MINUS:
		...
		if (context == EC_VALUE) {
			switch (expr->ex_type->ty_code) {
			case TY_LONGDOUBLE:
				opcode = OC_DOUBLE_UNARY_MINUS; /* FIX: */
				break;
			case TY_DOUBLE:
