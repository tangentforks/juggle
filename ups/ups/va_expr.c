...
char ups_va_expr_c_sccsid[] = "@(#)va_expr.c	1.26 24 May 1995 (UKC)";
...
struct drawst fdets;
{
	...
	edit_add_keymap_entry(keymap, 0, EDIT_DOWN_ARROW, inc_exprnum);
	
	edit_add_keymap_control_entry(keymap, 'p', dec_exprnum);
	edit_add_keymap_entry(keymap, 0, EDIT_UP_ARROW, dec_exprnum);
...
set_expr_result(ma, addr, args, nwords, p_res)
machine_t *ma;
taddr_t addr;
taddr_t *args;
int nwords;
taddr_t *p_res;
{
	...
	default:
		...
		errf("Can't display unknown type");
