...
char ups_tdr_c_sccsid[] = "@(#)tdr.c	1.19 04 Jun 1995 (UKC)";
...
char **args;
{
	...
	noload_flag = TRUE;
	add_to_load_path(*args, 1); /* noload */
	return 0;
}
...
char **args;
{
  ...
  fname = args[0];
  if (find_func_by_name(fname, &f, &f1) == 0)
  {
    add_breakpoint_object(f, (fil_t *)NULL, -1);
    if (f1)
      add_breakpoint_object(f1, (fil_t *)NULL, -1);
  }
...
char **args;
{
	...
	sw = Have_window ? get_current_srcwin() : NULL;
	show_var(sw, fil, lnum, varname, 0);
...
char **args;
{
	...
	if (strcmp(cmd, "execto") == 0)
		...
		Obj_to_make_visible = add_breakpoint_object(f, fil, lnum);
