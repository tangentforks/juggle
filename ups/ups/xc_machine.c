...
char ups_xc_machine_c_sccsid[] = "@(#)xc_machine.c	1.5 25 May 1995 (UKC)";
...
{ int i; for (i=LDOUBLE_NSLOTS; i>0; i--) PUSH(d.d_words[i], sp);}
#define POP_LDOUBLE(d, sp)	\
{ int i; for (i=0; i<LDOUBLE_NSLOTS; i++) d.d_words[i] = POP(sp);}
#else
#define PUSH_LDOUBLE(d, sp)	PUSH_DOUBLE(d, sp)
#define POP_LDOUBLE(d, sp)	POP_DOUBLE(d, sp)
#endif
#endif
