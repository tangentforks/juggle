...
char ups_va_val_c_sccsid[] = "@(#)va_val.c	1.39 04 Jun 1995 (UKC)";
...
static int get_fieldval PROTO((bool bits_big_endian, int word,
	...
	case TY_BITFIELD:
		if (DREAD_VAR(xp, addr, vl.vl_int)) {
			...
			if ((typecode == TY_ENUM || typecode == TY_U_ENUM) &&
				...
				int_to_string(buf, sizeof(buf), (vlong)val,
					      dv->dv_format);
			}
		}
	...
	case TY_U_ENUM:
		if (DREAD_VAR(xp, addr, vl.vl_int)) {
			if (dv->dv_format == DF_NONE) {
				enumval_to_string(buf, sizeof(buf),
						  vl.vl_int, type);
			}
			else {
				int_to_string(buf, sizeof(buf), (vlong)vl.vl_int,
					      dv->dv_format);
			}
		}
		break;
