...
char ups_ao_aout_c_sccsid[] = "@(#)ao_aout.c	1.2 24 May 1995 (UKC)";
 ...
 */
bool
aout_scan_main_symtab(textpath, fd, p_symtabs, p_symtab_cache, p_mainfunc)
const char *textpath;
int fd;
...
func_t **p_mainfunc;
{
	...
	if (!aout_scan_symtab(textpath, fd, (taddr_t)0, &st, p_mainfunc))
		return -1;
...
#ifdef COFF_SUN386
/*  This code has not been compiled recently and is probably broken.
 */
bool
coff_get_exec_info(textpath, fd, ei)
const char *textpath;
int fd;
Execinfo *ei;
{
	...
	ei->dynamic = aouthdr.vstamp == SUNVERSION_DYNAMIC;

	mem_text_offset = aouthdr.text_start;

	/*  The non-shlib case is documented in the SunOS coff(5) manual page.
	 ...
	 */
	file_text_offset = is_primary ? mem_text_offset - 0x1000 : 0;
 ...
 */
bool
aout_scan_symtab(textpath, fd, shlib_load_addr, p_symtab, p_mainfunc)
const char *textpath;
int fd;
taddr_t shlib_load_addr;
symtab_t **p_symtab;
func_t **p_mainfunc;
{
	...
	if (!scan_ao_symtab(textpath, fd, &eibuf, shlib_load_addr,
			   &st, &flist, p_mainfunc_name)) {
		return FALSE;
	}
