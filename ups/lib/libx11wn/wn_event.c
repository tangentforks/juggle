...
char wn_wn_event_c_sccsid[] = "@(#)wn_event.c	1.46 25 May 1995 (UKC)";
...
int
wn_expose_or_press_queued(wn, md, p_x, p_y, exposed, resized)
     int wn;
     int md;
     int *p_x;
     int *p_y;
     ...
     int *resized;
{
  ...
  if (_wn_Windowtab)
  {
    ...
    memset((char *)&ev, 0, sizeof(XMotionEvent));
    ev.type = KeyPress;
    ev.display = wn__Dpy;
