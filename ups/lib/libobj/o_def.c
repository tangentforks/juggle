...
char obj_def_sccsid[] = "@(#)o_def.c	1.14 09 Apr 1995 (UKC)";
...
int width;
{
	...
	for ( fldln = p_odesc->od_fldln; fldln; fldln=fldln->fl_next)
	{
	    ...
	    if ( elastic_fdesc )
	    {
		if (  total_width+elastic_fdesc->fd_resizes <= width )
		{
		    elastic_fdesc->fd_width = elastic_fdesc->fd_resizes;
		} else
		{
		    elastic_fdesc->fd_width = width - total_width;
